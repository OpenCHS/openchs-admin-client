...
class DateComponent extends Component {
      ...
      <div className="form-check form-check-inline" key={dateSubFieldId}>
        <label className="form-check-label">
          <input
            ...
            onChange={({ target }) => this.props.handleKeyValuesChange("durationOptions", dateSubField.value, target.checked, this.props.field)}
  ...
  render() {
    return (
      <FormGroup>
        {dateSubFields.map(dateSubField => this.renderDateSubField(dateSubField))}
      </FormGroup>
    );
  }
}
